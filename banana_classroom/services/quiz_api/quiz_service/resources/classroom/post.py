...
async def endpoint(body: BodyDict):
    ...
    if len(title) > 60:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="ensure this value has at most 60 characters")

    description = body.get("description", "")
    if not description:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Description is required")
    if len(description) > 1000:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="ensure this value has at most 1000 characters")
    ...
    if len(instructor) > 30:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="ensure this value has at most 30 characters")
