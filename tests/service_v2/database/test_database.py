...
class TestClassroomTable:
    ...
    def test_update_classroom(self,create_user):
        classroom = Classroom.safe_get(hash_key="test classroom")
        new_instructor = create_user(name="Mhell Bergonio",email="mhellbergonio@gmail.com",password="morfe123",role="instructor",created_at=self.time,updated_at=self.time)
        assert classroom
