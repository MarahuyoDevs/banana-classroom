...
class TestQuizCreation:
    ...
    def test_create_quiz_valid_data_no_questions(self, client: TestClient):
    ...
    def test_add_valid_question_to_existing_quiz(self, create_quiz, client: TestClient):
        ...
        response = client.post(f"/quiz/{quiz_id}/question", json=new_question_data)
    ...
    def test_add_question_with_missing_required_field(
        self, create_quiz, client: TestClient
    ):
    ...
    def test_modify_quiz_title(self, client: TestClient):
        # Create a quiz
        response = client.post(
            "/quiz",
            json={
        ...
        response = client.put(
            f"/quiz/{quiz_id}/modify_title", json={"title": new_title}
        )
    ...
    def test_modify_quiz_description(self, client: TestClient):
        # Create a quiz
        response = client.post(
            "/quiz",
            json={
                ...
                "questions": [
                    {
                        "description": "When was the Declaration of Independence signed?",
                        "answer": "1776",
                    },
                    {
